...
using NewTech.ProjectsNG.Startup;
using NewTech.ProjectsNG.Web.Authentication.JwtBearer;
using NewTech.ProjectsNG.Web.Authentication.TwoFactor;
using NewTech.ProjectsNG.Web.Chat.SignalR;
using NewTech.ProjectsNG.Web.Configuration;
using NewTech.ProjectsNG.Web.DashboardCustomization;

namespace NewTech.ProjectsNG.Web
{
    [DependsOn(
        typeof(ProjectsNGApplicationModule),
        typeof(ProjectsNGEntityFrameworkCoreModule),
        typeof(AbpAspNetZeroCoreWebModule),
        typeof(AbpAspNetCoreSignalRModule),
        typeof(ProjectsNGGraphQLModule),
        typeof(AbpRedisCacheModule), //AbpRedisCacheModule dependency (and Abp.RedisCache nuget package) can be removed if not using Redis cache
        typeof(AbpHangfireAspNetCoreModule) //AbpHangfireModule dependency (and Abp.Hangfire.AspNetCore nuget package) can be removed if not using Hangfire
    )]
    public class ProjectsNGWebCoreModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public ProjectsNGWebCoreModule(IWebHostEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(ProjectsNGApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(ProjectsNGWebCoreModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            IocManager.Resolve<ApplicationPartManager>()
                .AddApplicationPartsIfNotAddedBefore(typeof(ProjectsNGWebCoreModule).Assembly);
        }
